...
void SubghzApp_Init(void)
{
  ...
  /* Not used since it is Rx device */
  ...
  /* Start reception with infinite timeout value */
  Radio.Rx(0);
...
static void OnRxDone(uint8_t *payload, uint16_t size, int16_t rssi, int8_t LoraSnr_FskCfo)
{
  ...
  BufferRx[0] = (uint8_t)(rssi & 0xFF);
  BufferRx[1] = (uint8_t)(rssi >> 8 & 0xFF);

  /* Resend received message via UART */
  HAL_UART_Transmit(&huart1, BufferRx, size, 4000);
